...
const taskSource = { source: { queryName: "Table.Task" } } as any;
const DEFAULT_OPACITY = "0.8"; // formatting default barOpacity = 80

function makeId(index: number) {
    return host
        .createSelectionIdBuilder()
        .withCategory(taskSource, index)
        .createSelectionId();
}

function opacityOf(index: number): string {
    const r = element.querySelector(`rect.gantt-bar[data-dp-index="${index}"]`) as SVGRectElement | null;
...
beforeEach(() => {
    ...
    const dv = buildMockDataView({
        tasks: ["A", "B", "C"],
        startDates: ["2024-01-01", "2024-01-15", "2024-02-01"],
        endDates: ["2024-01-10", "2024-01-25", "2024-02-15"],
    });
    visual.update(makeUpdateOptions(dv));
});

describe("Gantt chart — bookmarks (registerOnSelectCallback)", () => {
    ...
    it("fireSelectCallback([idA, idC]) highlights only bars 0 and 2", () => {
        ...
        expect(opacityOf(0)).toBe(DEFAULT_OPACITY);
        expect(opacityOf(1)).toBe("0.3");
        expect(opacityOf(2)).toBe(DEFAULT_OPACITY);
    });

    it("fireSelectCallback([]) clears the highlight (all bars at default opacity)", () => {
        host.fireSelectCallback([makeId(0)]);
        expect(opacityOf(1)).toBe("0.3");
        host.fireSelectCallback([]);
        expect(opacityOf(0)).toBe(DEFAULT_OPACITY);
        expect(opacityOf(1)).toBe(DEFAULT_OPACITY);
        expect(opacityOf(2)).toBe(DEFAULT_OPACITY);
    });

    it("bookmark callback overrides a prior user click selection", async () => {
        const rectB = element.querySelector(`rect.gantt-bar[data-dp-index="1"]`) as SVGRectElement | null;
